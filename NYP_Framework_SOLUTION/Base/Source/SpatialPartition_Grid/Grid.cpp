...
void CGrid::SetDetailLevel(const CLevelOfDetails::DETAIL_LEVEL _theDetailLevel)
{
	...
	while (it != ListOfObjects.end())
	{
		...
		if (theEntity->GetLODStatus()) //if this entity in this grid has LOD status = true
		{
			theEntity->SetDetailLevel(_theDetailLevel);
		}
		++it;
	}
}
